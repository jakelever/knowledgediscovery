...
def calculateArrowsmithScore(x,z,neighbours):
	shared = neighbours[x].intersection(neighbours[z])
	return len(shared)
	
def calculateLinkingTermCountwithAMW(x,z,neighbours,cooccurrences):
	linkingTermCount = calculateArrowsmithScore(x,z,neighbours)
	amw = calculateAverageMinimumWeight(x,z,neighbours,cooccurrences)
...
if __name__ == '__main__':
	...
	with open(args.outFile,'w') as outF:
		for i,(x,z) in enumerate(relationsToScore):
			...
			ltc_amwScore = calculateLinkingTermCountwithAMW(x,z,neighbours,cooccurrences)

			outData = [x,z,ltc_amwScore]
