...
python generateDetritusTestcases.py --$type --skeletonSentencesFile $skeletonSentences --wordlistFile $wordlist --detritusFile $detritusWordlist --outXML_prefix $goldPrefix  --outDetritusCounts $goldDetritus > $generationLog
if [[ $? -ne 0 ]]; then
...
echo "Running code..."
python ../detritusMajigger.py --termsWithSynonymsFile $wordlist --articleFilelist $goldArticleList --outFile $processedDetritus > $detritusLog 2>&1
if [[ $? -ne 0 ]]; then
