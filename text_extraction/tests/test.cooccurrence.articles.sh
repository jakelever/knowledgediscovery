...
goldPrefix=$workingDir/gold.articles.
goldArticleList=$workingDir/gold.articleList.txt
...
type=generateArticles

generationLog=$workingDir/generationLog.txt
cooccurrenceLog=$workingDir/cooccurrenceLog.txt

echo "Generating test data..."
python generateTestcases.py --$type --skeletonSentencesFile $skeletonSentences --wordlistFile $wordlist --outXML_prefix $goldPrefix  --outCooccurrences $goldCooccurrences --outExplanations $goldExplanations > $generationLog

find $goldPrefix* -type f > $goldArticleList

echo "Running code..."
python ../cooccurrenceMajigger.py --termsWithSynonymsFile $wordlist --articleFilelist $goldArticleList --outFile $processedCooccurences > $cooccurrenceLog 2>&1
